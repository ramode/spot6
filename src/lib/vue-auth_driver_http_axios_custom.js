module.exports = {
  ...
  _invalidToken: function (res) {
    // Чтобы отбросить ложные срабатывания по 401'ые отбивкам от Яндексовского API
    if (!res.config.url.startsWith('http://rebilling.eth0.pro/api/v1/ym/') && res.status === 401) {
      return true
    }
  },
